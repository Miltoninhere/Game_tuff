...
def minimizar_ventana():
    root.withdraw()  
    minimized_window = tk.Toplevel(root)
    minimized_window.title("Minimizado")
    minimized_window.geometry("40x40")
    minimized_window.resizable(False, False)

    def restaurar_ventana():
        root.deiconify() 
        minimized_window.destroy()

    btn_abrir = tk.Button(minimized_window, text="Abrir", command=restaurar_ventana)
    btn_abrir.pack()

btn_minimizar = tk.Button(frame_botones, text="Min", command=minimizar_ventana, font=("Arial", 10))
btn_minimizar.pack(side=tk.LEFT, padx=5) 
